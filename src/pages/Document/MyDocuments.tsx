...
const MyDocuments: React.FC = () => {
  ...
  // Auto-deselect documents that load as Approved or In Progress
  useEffect(() => {
  ...
  useEffect(() => {
    if (appliedDepartment || appliedSubDepartment || appliedStartDate || appliedEndDate) {
      setCurrentPage(1);
      setSelectedDocIds(new Set()); // Reset selection when filters change
    }
  ...
  const handleSelectAll = useCallback((selected: boolean) => {
    if (selected) {
      // Only select IDs from paginatedDocs that aren't Approved or In Progress
      const eligibleIds = paginatedDocs
        .filter(d => {
          const id = d.newdoc?.ID?.toString();
          const status = docStatuses[id];
          return status !== 'approved' && status !== 'in_progress';
        })
        .map(d => d.newdoc?.ID?.toString())
      ...
      const currentPageIds = paginatedDocs.map(d => d.newdoc?.ID?.toString());
      setSelectedDocIds(prev => {
        const newSet = new Set(prev);
        currentPageIds.forEach(id => {
          if (id) newSet.delete(id.toString());
        });
        return newSet;
      });
    }
  ...
  const handleBulkRequestApproval = async () => {
    ...
    try {
      ...
      // Convert to array and send all requests in parallel for better performance
      const idArray = Array.from(selectedDocIds);
      console.log(`📤 [BULK APPROVAL] Sending ${idArray.length} parallel requests...`);
      const requestStart = performance.now();

      // Use Promise.allSettled to send all requests concurrently
      const responses = await Promise.allSettled(
        idArray.map(id => requestDocumentApproval(Number(id)))
      );

      console.log(`✅ [BULK APPROVAL] Requests done in ${(performance.now() - requestStart).toFixed(0)}ms`);
      ...
      responses.forEach((result, index) => {
        if (result.status === 'fulfilled' && result.value?.success) {
          ...
          results.failed++;
          console.error(`Failed to request approval for doc ${idArray[index]}:`,
            result.status === 'rejected' ? result.reason : result.value);
        }
      });

      if (results.success > 0) {
        toast.success(`Successfully sent ${results.success} approval request(s).${results.failed > 0 ? ` (${results.failed} failed)` : ''}`, { id: toastId });
        setSelectedDocIds(new Set()); // Clear selection on success

        console.log('🔄 [BULK APPROVAL] Refreshing documents...');
        const refreshStart = performance.now();

        // Force immediate refresh to update document statuses
        await loadDocuments();

        console.log(`✅ [BULK APPROVAL] Refresh done in ${(performance.now() - refreshStart).toFixed(0)}ms`);
        console.log(`🏁 [BULK APPROVAL] Total: ${((performance.now() - startTime) / 1000).toFixed(2)}s`);
      } else {
        toast.error(`Failed to send approval requests.${results.failed > 0 ? ` (${results.failed} errors)` : ''}`, { id: toastId });
      }
    } catch (error) {
      console.error('Bulk approval error:', error);
      toast.error('An unexpected error occurred during bulk approval.', { id: toastId });
    } finally {
    ...
    paginatedDocs.map((document) => {
      ...
      return (
        <DocumentCard
          ...
          selected={selectedDocIds.has(doc.ID?.toString())}
          onSelect={handleSelectDocument}
          onStatusUpdate={handleStatusUpdate}
        />
      );
    }), [paginatedDocs, allocationPermissions, navigate, handleDelete, debouncedSearchTerm, department, subDepartment, startDate, endDate, appliedDepartment, appliedSubDepartment, appliedStartDate, appliedEndDate, currentPage, selectedDocIds, handleSelectDocument, handleStatusUpdate]
  );
